...
def quick_evaluate():
    ...
    # CNNLSTM
    if os.path.exists("checkpoints/cnnlstm_model.pth"):
        try:
            model = CNNLSTM(frame_shape=frame_shape, time_steps=time_steps)
            model.load_state_dict(torch.load("checkpoints/cnnlstm_model.pth", map_location=device))
            model.to(device)
            model.eval()
    ...
    # PretrainedCNNLSTM
    if os.path.exists("checkpoints/pretrained_cnnlstm_model.pth"):
        try:
            ...
            model.load_state_dict(torch.load("checkpoints/pretrained_cnnlstm_model.pth", map_location=device))
            model.to(device)
            model.eval()
    ...
    # SimpleResNet
    if os.path.exists("checkpoints/simple_resnet_model.pth"):
        try:
            model = SimpleResNet(frame_shape=frame_shape)
            model.load_state_dict(torch.load("checkpoints/simple_resnet_model.pth", map_location=device))
            model.to(device)
            model.eval()
