...
To rigorously assess the generalizability of our physics-informed models, we employ a Leave-One-Sequence-Out (LOSO) cross-validation strategy. Unlike standard random splitting, which can lead to over-optimistic performance due to temporal correlation within the same experimental run, LOSO ensures that the model is tested on entirely unseen experimental subjects. We iterate through each of the seven distinct experimental sequences in our dataset, holding one out as the test set while training on the remaining six. This approach provides a true measure of the model's ability to normalize across subject-specific variants in tissue properties and ultrasonic artifacts.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{paper/figures/edge_deployment_fps.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{paper/figures/accuracy_thresholds.png}
