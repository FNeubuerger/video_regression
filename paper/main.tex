...
We also export trained models to the ONNX format using the repository's export pipeline (e.g. \texttt{benchmarks/convert_to_onnx.py}) and benchmark them with ONNX Runtime using \texttt{benchmarks/benchmark_deployment.py}. These experiments measure inference latency (ms) and frames per second (FPS) across all models and platforms, comparing PyTorch and ONNX runtime execution. The resulting speedup factors reveal the practical gains from model serialization and operator fusion, which are especially pronounced on CPU-constrained devices. Detailed results are recorded in CSV format for aggregation and analysis via \texttt{benchmarks/analyze_results.py}.
