    ...
    * Target rank greater than outputs -> take last frame of target.
    ...
    if outputs.dim() == 3:
        if outputs.shape[1:] == (4, 4):
            ...
            outputs.shape[-1] <= 8
            and outputs.shape[-2] <= 8
            and outputs.shape[-2] != outputs.shape[-1]
        ):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_progress):
            ...
            if isinstance(batch, (list, tuple)):
                if len(batch) >= 4:
                    images, labels_raw, mask, scalars = (
                        batch[0],
                        batch[1],
                        batch[2],
                        batch[3],
            ...
            flow = None
            if (
                isinstance(criterion_instance, PhysicsInformedLoss)
                and images.shape[2] == 5
            ):
            ...
            if scaler is not None:
                with autocast("cuda"):
                    ...
                    if isinstance(kl_loss, torch.Tensor) or kl_loss > 0:
                        loss = (
                            loss + 0.1 * kl_loss
                        )  # Default weight 0.1 for generic training
                ...
                torch.nn.utils.clip_grad_norm_(
                    model_instance.parameters(), max_norm=1.0
                )
            ...
            for batch in val_progress:
                ...
                if is_scalar_model:
                    if scalars is not None:
                        ...
                        # Fix: Handle dimension mismatch for models expecting single frame output
                        if (
                            "physicscnnlstm" not in model_name_lower
                            and labels.dim() == 3
                        ):
                            labels = labels[:, -1, :]  # Take last time step -> (B, 4)
                    else:
                ...
                if scaler is not None:
                    with autocast("cuda"):
                        ...
                        if isinstance(criterion_instance, AdvancedBioHeatLoss):
                            ...
                            flow = (
                                images[:, :, 3:, :, :] if images.shape[2] >= 5 else None
                            )
                            labels_phys = _pool_target_for_bioheat(
                                labels.float(), criterion_instance
                            )
                            ...
                            if outputs.dim() == 3 and outputs.shape[1:] == (4, 4):
                                outputs = outputs.mean(dim=(1, 2))
                            loss = criterion_instance(
                                outputs, labels.float(), mask=mask
                            )
                        else:
                            outputs, target = align_outputs_target(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train all temperature regression models"
    )
    parser.add_argument(
        "--epochs",
        type=int,
        default=config["training_params"].get("epochs", 50),
        help="Maximum number of epochs",
    )
    parser.add_argument(
        "--patience",
        type=int,
        default=config["training_params"].get("patience", 10),
        help="Early stopping patience",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
        default=config["training_params"].get("batch_size", 128),
        help="Batch size",
    )
    parser.add_argument(
        "--models",
    ...
    parser.add_argument(
        "--masked", action="store_true", help="Enable thermometer artifact masking"
    )
    parser.add_argument(
    ...
    for model_type in models_to_train:
        ...
        if os.path.exists(model_path) and not should_rerun:
            print(
                f"Model {model_type} already exists at {model_path} and force_rerun is False. Skipping."
            )
            continue
    ...
    for model_type, result in results.items():
        if "error" in result:
            ...
            print(
                f"{model_type:20s}: {result['epochs']:3d} epochs, "
