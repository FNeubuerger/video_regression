"""
Training script for Bayesian Spatial Convection PINN.
Combines Bayesian Neural Networks (Uncertainty) with Spatial Convection Physics.
"""
...
from models.bayesian import BayesianSpatialResNet
from physics.bioheat_loss import AdvancedBioHeatLoss
from tqdm import tqdm
import wandb
import torchbnn as bnn

def train_bayesian_spatial_convection(epochs=50, batch_size=32, learning_rate=1e-4, kl_weight=0.1):
    # Initialize WandB
    wandb.init(project="video-temperature-regression", name="bayesian-spatial-convection")
    
    # Config
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    sequence_length = 1 # Spatial model uses single frame
    
    print(f"Training Bayesian Spatial Convection PINN on {device}")
    ...
    # Model
    print("Initializing BayesianSpatialResNet...")
    # Input: 5 channels (3 RGB + 2 Flow)
    model = BayesianSpatialResNet(frame_shape=(64, 64, 5)).to(device)
    ...
    for epoch in range(epochs):
        ...
        for images, labels in progress:
            ...
            # Remove time dim for model if needed, but model handles it
            # Model expects (B, T, C, H, W) or (B, C, H, W)
            # Dataset returns (B, 1, 5, 64, 64)
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "models/bayesian_spatial_convection.pth")
