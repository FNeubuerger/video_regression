...
def main():
    ...
    parser.add_argument('--model', required=True, help='Path to trained temperature map model')
    parser.add_argument('--cem43_thresh', type=float, default=60.0, help='CEM43 threshold for ablation zone')
    ...
    def predict_temperature_maps(video_path):
        ...
        while True:
            ...
            temp_map = gray / 255.0 * 80  # Fake: scale to 0-80°C
    ...
    for v_path in tqdm(video_files):
        ...
        temp_seq = predict_temperature_maps(v_path)  # shape (T, H, W)
        cem43_map = calculate_cem43(temp_seq, dt=1/30)  # dt: 1/30 min per frame (30 fps)
        ...
        for img_path in img_paths:
            ...
            vis = np.stack([pred_mask.astype(np.uint8)*255, gt_mask.astype(np.uint8)*255, np.zeros_like(gt_mask)], axis=-1)
            out_path = os.path.join(args.output_dir, f'vis_{base}_{os.path.basename(img_path)}.png')
            cv2.imwrite(out_path, vis)
        # Average metrics across views
        results.append({'video': base, 'iou_mean': np.mean(iou_list), 'dice_mean': np.mean(dice_list), 'iou_views': iou_list, 'dice_views': dice_list})
    # Save results
    import csv
